                    ...
                    <div class="panel-heading" id="accordion">
                        ...
                        <form id="daddy-shoutbox-form" action="../json/daddy_shoutbox_json.php?action=add" method="post" style="width:100%;"> 
                            <div class="input-group">
                                <input type="hidden" name="nickname" value="<?=$auth->userNoRegister?>" />
                                <input type="hidden" name="reqId" value="<?=$auth->userPelamarId?>" readonly /> 
					...
					<script type="text/javascript">
                          ...
                          function prepare(response) {
                            ...
                            var string = '<div class="shoutbox-list" id="list-'+count+'">'
                                ...
                                +'</div>';
                            if(response.nickname == '<?=$auth->userNoRegister?>')
                            {						
                                var string = '<li class="right clearfix">' +
                                    '<span class="chat-img pull-right">' +
                                        '<img src="images/chat-me.png" class="img-circle" />' +
                                    '</span>' +
                            ...
                            {                                						
                                var string = '<li class="left clearfix">' +
                                    '<span class="chat-img pull-left">' +
                                        '<img src="images/chat-u.png" class="img-circle" />' +
                                    '</span>' +
                                    '<div class="chat-body clearfix">' +
                          ...
                          function refresh() {
                            $.getJSON("../json/daddy_shoutbox_json.php?reqId=<?=$auth->userPelamarId?>&action=view&time="+lastTime, function(json) {
                              if(json.length) {
