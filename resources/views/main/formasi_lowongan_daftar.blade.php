...
use App\Models\FormasiDetil;

$set= new Pelamar();

$reqRowId= request()->reqRowId;
$reqId= $auth->userPelamarEnkripId;
...
if($tempIsKirimLamaran == "1")
{
	...
	echo 'top.location.href = "index.phpapp/index/formasi_lowongan";';
	echo '</script>';
	exit();
}
...
//elseif($tempJejakDataPribadi == "icon-belum" || $tempJejakPendidikanFormal == "icon-belum" || $tempJejakPengalamanBekerja == "icon-belum" || $tempJejakPelatihan == "icon-belum" || $tempJejakArahMinat == "icon-belum")
{
	...
	echo 'top.location.href = "app/index/formasi_lowongan";';
	echo '</script>';
	exit();
}
	...
	$(function(){
		...
		$('#ff').form({
			...
			success:function(data){
				...
				$.messager.alertLink('Info', "Anda Sukses Mengirim Lamaran. Cek Email Anda Sebagai Bukti Anda Telah Mengirim Lamaran", 'info', 'app/index/formasi_lowongan');
				//$.messager.alert('Info', data, 'info');
				//data = data.split("-");
				//$.messager.alert('Info', data[1], 'info');
				//$('#rst_form').click();
				// document.location.href = 'app/index/formasi_lowongan';
			}
		});
    ...
    <div id="pendaftaran">
        ...
        <table>
            ...
            <tr id="reqSimpanInfo" style="display:none">
            	<td colspan="2"><p class="keterangan">Dengan klik tombol SETUJU, Saya menyatakan telah membaca dan memahami seluruh petunjuk serta mengijinkan Panitia Seleksi untuk menggunakan data administrasi dalam proses Seleksi PTPN 1</p></td>
            </tr>
