...
class Informasi extends Entity{ 
    ...
    **/
	...
	function insertData()
	{
		...
		$str = "INSERT INTO informasi(informasi_id, user_login_id, nama, tanggal, keterangan, status_halaman_depan, status_aktif, status_informasi)
				VALUES(
    ...
    **/ 
	...
	function selectByParamsLike($paramsArray=array(),$limit=-1,$from=-1)
	{
		$str = "SELECT i.informasi_id, i.user_login_id, i.nama as i_nama, i.tanggal, i.keterangan, i.status_halaman_depan, i.status_aktif, i.status_informasi, i.link_file,
				FROM informasi i WHERE informasi_id IS NOT NULL "; 
    ...
    **/ 
	...
	{
		$str = "SELECT COUNT(i.informasi_id) AS ROWCOUNT FROM informasi i WHERE i.informasi_id IS NOT NULL"; 
		foreach ($paramsArray as $key => $val)
		{
			$str .= " AND $key = '$val' ";
		}
