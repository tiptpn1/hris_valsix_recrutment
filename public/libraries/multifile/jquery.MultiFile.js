...
function uploadProgress(element,index,jenisDokumen)
{
	//configuration
	...
	if(!window.File && window.FileReader && window.FileList && window.Blob){ //if browser doesn't supports File API
		...
		tokenData =  $('meta[name="csrf-token"]').attr('content');
		formData.append('reqLampiran', element.files[0]);
		formData.append('reqNamaDokumen', jenisDokumen);
		...
		$.ajax({
			...
			var div = $('#reqDivLampiranSuratLamaran').html();
			...
			{
				$("#reqLampiran"+index+"Data").val(obj.message);
				$("#progressBar"+index).hide();
				$("#spanFile"+index).hide();
				$("#spanFile"+index).html("");
				$("#spanInfo"+index).show();
				$("#tdFile"+index).html("<a href='uploads/"+obj.message+"' target='_blank'><i class='fa fa-download'></i>&nbsp;download</a>");
				
				//$("#browseFile"+index).html("<span>Dokumen berhasil diupload.</span>");
			}
		});
	}
}